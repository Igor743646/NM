...
public: 
    ...
    MCFDES(const FDESbase& _p) : FDESbase(_p) {
        probabilities = _make_prob();
    }

    void solve(ull count = 1000) {
    ...
    void solve_GPU(ull count = 1000) {
        ...
        setup_kernel<<<512, 512>>>(dev_states, grid_size);
        CSC(cudaGetLastError());

        solve_kernel<<<512, 512>>>(dev_states, result_gpu, prefsum_prob_gpu, n, k, count, L, h, tau, gamma, borders_gpu, source_gpu);
        CSC(cudaGetLastError());
