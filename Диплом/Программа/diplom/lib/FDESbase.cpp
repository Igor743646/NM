...
is_borders(p.is_borders), n(p.n), k(p.k)
{
    result = p.result;
    memo_g_alpha[0] = 1.0;
    memo_g_gamma[0] = 1.0;
...
void FDESbase::Info() {
    ...
    Output::print("\tN = ", n);
    Output::print("\tT = ", k);
    Output::print("\tD * (tau^gamma) / (h^alpha) = ", D(0) * std::pow(tau, gamma) / std::pow(h, alpha));
    Output::print("\tgamma / alpha = ", gamma / alpha);

    if (D(0) * std::pow(tau, gamma) / std::pow(h, alpha) > gamma / alpha) {
        Output::print("May be problem with condition");
        Output::print(D(0) * std::pow(tau, gamma) / std::pow(h, alpha), " ", gamma / alpha);
        Output::print("You may make tau <= ", std::pow(gamma * std::pow(h, alpha) / alpha / D(0), 1.0/gamma));
        Output::print("Or make h >= ", std::pow(std::pow(tau, gamma) * alpha * D(0) / gamma, 1.0/alpha));
...
double FDESbase::a(double x) {
    return (1.0 + beta) * (D(x) / 2.0) * (std::pow(tau, gamma) / std::pow(h, alpha));
}

double FDESbase::b(double x) {
    return (1.0 - beta) * (D(x) / 2.0) * (std::pow(tau, gamma) / std::pow(h, alpha));
}

double FDESbase::c(double x) {
    return V(x) * std::pow(tau, gamma) / 2 / h;
}
